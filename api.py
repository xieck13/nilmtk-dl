    ...
    def train_jointly(self,clf,d):
        ...
        for dataset in d:
            ...
            for building in d[dataset]['buildings']:
                ...
                main_df = next(main_meter.load(physical_quantity='power', ac_type=self.power['mains'], sample_period=self.sample_period))
                ...
                for appliance_name in self.appliances:
                    ...
                    app_df = next(app_meter.load(physical_quantity='power', ac_type=self.power['appliance'], sample_period=self.sample_period))
                ...
                if self.DROP_ALL_NANS:
                    main_df_list, appliance_readings = self.dropna(main_df_list, appliance_readings) # Ttrain_list: [pd[sec],pd[sec]..]
    ...
    def test_jointly(self,d):
        ...
        for dataset in d:
            ...
            for building in d[dataset]['buildings']:
                ...
                main_df=next(test_meter.load(physical_quantity='power', ac_type=self.power['mains'], sample_period=self.sample_period))
    ...
    def call_predict(self,classifiers):
        ...
        for i in gt_overall.columns:
            for clf in pred_overall:
                if(os.path.exists('result/'+self.storing_key+'/'+str(i)+'/'+str(clf)+'/section_image') == False):
        ...
        for i in gt_overall.columns:
            ...
            for j, gt in enumerate(gt_overall_list):
                for clf in pred_overall:
                    ...
                    plt.savefig('result/'+self.storing_key+'/'+str(i)+'/'+str(clf)+'/section_image/'+str(j)+'.png')
